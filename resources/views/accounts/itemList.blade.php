    ...
    <div class="container">
        <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
            ...
            <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0">
                ...
                <li><a href="./index.php?action=item_list" class="nav-link px-2 text-secondary">アイテム一覧</a></li>
...
<table class="table table-bordered">
    ...
    <?php
    if (!empty($item)) {//コントローラーから渡された$usersを使って表示
        foreach ($item as $items) {
            echo '<tr>' . '<td>' . $items['id'] . '</td>' . '<td>' . $items['name'] . '</td>' . '<td>' . $items['type'] . '</td>' . '<td>' . $items['Effect_value'] . '</td>' . '<td>' . $items['explanation'] . '</td>' . '</tr>';
        }
    }
    ?>
</table>
